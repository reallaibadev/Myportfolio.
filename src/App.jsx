...
import { 
  ...
  Sparkles, Award, Cpu, Star, ExternalLink, Globe
} from 'lucide-react';
...
export default function App() {
  ...
  const personalInfo = {
    ...
    // Expanded Project Links from User Vercel Deployments
    projects: [
      {
  ...
  const getBotResponse = (input) => {
    ...
    if (text.includes('project') || text.includes('chess') || text.includes('game') || text.includes('vercel') || text.includes('links')) {
      return "Laiba has multiple impressive projects hosted on Vercel, including TrekBag Travel Assistant, an Interactive Periodic Table, Word Analytics Dashboard, and several responsive landing pages. You can browse them below!";
    }
    ...
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
          ...
          <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
            ...
            <div className="grid grid-cols-3 gap-4 max-w-md mx-auto lg:mx-0 pt-2">
              <div className={`p-3 rounded-2xl border ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'} text-center`}>
                <span className="text-2xl font-black text-sky-500 block">10+</span>
                <span className="text-xs text-slate-400 uppercase font-bold">Live Projects</span>
              </div>
              <div className={`p-3 rounded-2xl border ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'} text-center`}>
                <span className="text-2xl font-black text-indigo-500 block">11+</span>
      ...
      <section id="projects" className="py-20 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
            ...
            <div className="lg:col-span-4 space-y-6">
              <div className={`p-6 sm:p-8 rounded-3xl border ${darkMode ? 'bg-slate-900/70 border-slate-800' : 'bg-white border-slate-200'}`}>
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-indigo-500/10 text-indigo-400 mb-4">
                  <Star size={12} /> Live Interactive Sandbox
                </div>
          ...
          {/* Vercel Projects Gallery */}
          <div className="mt-20">
            <div className="text-center max-w-2xl mx-auto mb-10 space-y-2">
              <span className="text-xs font-bold tracking-widest text-sky-500 uppercase">Interactive Vercel Applications</span>
              <h3 className="text-2xl sm:text-3xl font-black">My Deployed Projects</h3>
              <p className="text-slate-400 text-xs sm:text-sm">Click any project to inspect details or open live deployed production links.</p>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-2 mb-8">
              {[
                ...
                { label: 'React Web Apps', value: 'react' },
                { label: 'Interactive Tools', value: 'tools' },
                { label: 'Creative UIs', value: 'design' }
              ].map((btn) => (
                <button
                  key={btn.value}
                  onClick={() => setActiveProjectTab(btn.value)}
                  className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all ${
                    activeProjectTab === btn.value 
                      ? 'bg-gradient-to-r from-sky-500 to-indigo-600 text-white shadow-md shadow-indigo-500/20' 
                      : darkMode 
                        ? 'bg-slate-900 hover:bg-slate-800 text-slate-400 hover:text-white' 
                        : 'bg-white hover:bg-slate-100 text-slate-600 hover:text-slate-900 border border-slate-200'
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProjects.map((proj, idx) => (
                <div 
                  key={idx}
                  className={`flex flex-col justify-between p-6 rounded-3xl border transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl ${
                    darkMode 
                      ? 'bg-slate-900/40 border-slate-800/80 hover:bg-slate-900/80 hover:border-slate-700' 
                      : 'bg-white border-slate-200 hover:shadow-indigo-500/5'
                  }`}
                >
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-sky-500/10 to-indigo-600/10 flex items-center justify-center text-sky-400">
                        <Globe size={18} />
                      </div>
                      <span className="text-[10px] font-bold text-sky-400 bg-sky-500/10 px-2 py-0.5 rounded-full uppercase tracking-wider">
                        {proj.category}
                      </span>
                    </div>

                    <h4 className="font-bold text-lg mb-2">{proj.title}</h4>
                    <p className={`text-xs sm:text-sm mb-4 leading-relaxed ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                      {proj.desc}
                    </p>
                  </div>
                  ...
                  <div>
                    <div className="flex flex-wrap gap-1.5 mb-5">
                      {proj.tech.map((t, index) => (
                        <span key={index} className="text-[10px] bg-slate-800/50 text-slate-400 px-2 py-0.5 rounded-md font-medium">
                          {t}
                        </span>
                      ))}
                    </div>
                    ...
                    <a 
                      ...
                      className="w-full py-2.5 px-4 rounded-xl text-xs font-semibold bg-sky-500 hover:bg-sky-600 text-white transition-all flex items-center justify-center gap-1.5 shadow-lg shadow-sky-500/10"
                    >
                      View Live Deployment <ExternalLink size={13} />
                    </a>
                  </div>
                </div>
              ))}
